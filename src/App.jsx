...
function saveLocal(key, val) {
  try {
    if (val === null || val === undefined) {
      localStorage.removeItem("BW_" + key);
    } else {
...
export default function App() {
  ...
  const [screen, setScreen] = useState(() => {
    ...
    if (savedUser && savedUser.id && savedUser.email) return "app";
    return loadLocal("screen", "login");
  });
    ...
    <div style={{ display: "flex", height: "100vh", width: "100vw", background: "var(--bg)", overflow: "hidden" }}>
      ...
      <div style={{ width: sideOpen ? 230 : 60, background: "var(--surf)", borderRight: "1px solid var(--bdr)", height: "100vh", display: "flex", flexDirection: "column", transition: "width .22s ease", overflowX: "hidden", flexShrink: 0 }}>
        ...
        <div style={{ padding: "12px 10px", borderTop: "1px solid var(--bdr)", flexShrink: 0 }}>
          ...
          {sideOpen && (
            <button
              style={{ marginTop: 8, width: "100%", padding: "5px", background: "transparent", border: "1px solid var(--bdr)", borderRadius: 6, color: "var(--sub)", fontSize: 11, cursor: "pointer" }}
              onClick={() => {
                setUser(null);
                setScreen("login");
                saveLocal("user", null);
                saveLocal("screen", "login");
              }}
            >
              Sign Out 🌀
            </button>
